          ...
          <ResponsiveContainer width="100%" height="100%">
            ...
            content={({ active, payload }) => {
              if (active && payload && payload.length) {
                const { approved_date,total_loan_amount, loan_count } = payload[0].payload;
                return (
                  <div className="custom-tooltip bg-white border border-gray-300 p-2 rounded shadow-lg">
                    <p className="label">{`Approved: ${approved_date}`}</p>
                    <p className="desc">{`Total Loan Amount: ${formatCurrency(total_loan_amount)}`}</p>
                    <p className="desc">{`No.of Loans: ${loan_count}`}</p>
                  </div>
                );
              }
